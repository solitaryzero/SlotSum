...
# import wandb
# wandb.init(project="slotsum-raw-bart")
...
def main(params):
    ...
    def tokenize_function(examples):
        ...
        target_text = examples['sloted_text']
    ...
    predictions = trainer.predict(
        ...
        max_length=params['res_max_length'],
        num_beams=5,
    )
    p = np.where(predictions.predictions != -100, predictions.predictions, tokenizer.pad_token_id)
    decoded_preds = tokenizer.batch_decode(p, skip_special_tokens=True)
    # decoded_preds = ["\n".join(nltk.sent_tokenize(pred.strip())) for pred in decoded_preds]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_path', type=str, default='./models/slotsum_template')
    parser.add_argument('--output_path', type=str, default='./data/slotsum/template_prediction')
    parser.add_argument('--base_model', type=str, default="./models/pretrained/bart-large")
