...
import wandb
wandb.init(project="slotsum-filler")
...
def main(params):
    ...
    def tokenize_function(examples):
        ...
        inputs = []
        for s, t, h in zip(source_text, title, header):
            inputs.append([t+' '+h, s])

        model_inputs = tokenizer(inputs, padding='max_length', truncation=True)
        # labels = tokenizer(content, padding='max_length', truncation=True)
        labels = tokenizer(content, padding='longest', truncation=True)
    ...
    trainer = Seq2SeqTrainer(
        ...
        train_dataset=tokenized_dataset['train'].shard(10, 1).shuffle(seed=params['seed']),
...
if __name__ == "__main__":
    ...
    # data paths
    parser.add_argument('--data_path', type=str, default='./data/slotsum/filler')
    parser.add_argument('--output_path', type=str, default='./models/slotsum_filler')
