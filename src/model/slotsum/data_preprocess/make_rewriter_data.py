...
def get_rewriter_data(
    ...
    for header, content in table:
        ...
        if (header in slot_names):
            ...
            fact_list.append([header, content])

    out_json = {
        'title': in_json['title'],
        'target_text': in_json['target_text'],
        'source_text': in_json['source_text'],
        'sloted_text': in_json['sloted_text'],
        'facts': fact_list,
    }
...
if __name__ == '__main__':
    dataset_path = './data/slotsum/sloted'
    out_path = './data/slotsum/rewriter'
        ...
        with open(full_in_path, 'r', encoding='utf-8') as fin:
            with open(full_out_path, 'w', encoding='utf-8') as fout:
                for line in tqdm(fin.readlines()):
                    ...
                    rewriter_json = get_rewriter_data(
                        js,
                        slot_names=slot_names,
                        debug=False,
                    )

                    json.dump(rewriter_json, fout)
                    fout.write('\n')